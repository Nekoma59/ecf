...
<section class="order-details">
   ...
   <?php
      ...
      if($select_orders->rowCount() > 0){
         while($fetch_order = $select_orders->fetch(PDO::FETCH_ASSOC)){
            $select_film = $conn->prepare("SELECT * FROM `film` WHERE id = ? LIMIT 1");
            $select_film->execute([$fetch_order['film_id']]);
            if($select_film->rowCount() > 0){
               while($fetch_film = $select_film->fetch(PDO::FETCH_ASSOC)){
                  $sub_total = ($fetch_order['price'] * $fetch_order['seats']);
   ...
   <div class="box">
      <div class="col">
         ...
         <p class="price"><i class="fas fa-dollar-sign"></i> <?= $fetch_order['price']; ?> x <?= $fetch_order['seats']; ?></p>
      ...
      <div class="col">
         ...
         <?php if($fetch_order['status'] == 'canceled'){ ?>
            <a href="checkout.php?get_id=<?= $fetch_film['id']; ?>" class="btn">order again</a>
         <?php }else{ ?>
         <form action="" method="POST">
            <input type="submit" value="cancel order" name="cancel" class="btn" onclick="return confirm('cancel this order?');">
         </form>
         <?php } ?>
      </div>
   </div>
