...
if(isset($_POST['update_cart'])){
   ...
   $update_seats= $conn->prepare("UPDATE `cart` SET seats = ? WHERE id = ?");
   $update_seats->execute([$seats, $cart_id]);
...
<section class="films">
   ...
   <form action="" method="POST" class="box">
      ...
      <div class="flex">
         ...
         <input type="number" name="seats" required min="1" value="<?= $fetch_cart['seats']; ?>" max="99" maxlength="2" class="seats">
         <button type="submit" name="update_cart" class="fas fa-edit">
         </button>
      </div>
      <p class="sub-total">sub total : <span><i class="fas fa-dollar-sign"></i> <?= $sub_total = ($fetch_cart['seats'] * $fetch_cart['price']); ?></span></p>
      <input type="submit" value="delete" name="delete_item" class="btn" onclick="return confirm('delete this item?');">
   </form>
   <?php
      $grand_total += $sub_total;
      }else{
         echo '<p class="empty">film was not found!</p>';
      }
      }
   }else{
   ...
   <?php if($grand_total != 0){ ?>
      <div class="cart-total">
         ...
         <form action="" method="POST">
          <input type="submit" value="empty cart" name="empty_cart" class="btn" onclick="return confirm('empty your cart?');">
         </form>
